...
def run_ocr(image, prompt_label, custom_prompt):
    ...
    prompt_text = (
        ...
        else PROMPT_MODES.get(prompt_label, PROMPT_MODES["Parsing completo (testo + layout)"])
    )
...
with gr.Blocks(title="dots.ocr UI", theme=gr.themes.Soft()) as demo:
    ...
    with gr.Row():
        status_box = gr.Textbox(
            label="Stato server", value=server_status(), interactive=False, scale=3
        )
    ...
    with gr.Row():
        ...
        with gr.Column(scale=1):
            ...
            output_raw = gr.Textbox(
                label="Risultato (testo grezzo)", lines=20, show_copy_button=True
            )
