...
class Movimento:
    ...
    data_valuta: str | None  # DD.MM.YYYY o None
    descrizione_raw: str  # descrizione originale (pre-pulizia)
    descrizione: str  # descrizione pulita (post-pulizia)
    dare: float | None = None
    avere: float | None = None
    causale: str | None = None  # codice causale (es. "31", "78")
    causale_nome: str | None = None  # nome causale (es. "Pagamento disposiz. elettroniche")
    pagina: int = 0
...
class BankTemplate(ABC):
    ...
    def _ha_tabella_movimenti(self, html: str) -> bool:
        ...
        for table in tables:
            ...
            has_importo = any(
                h in ("addebiti", "addebita", "accrediti", "accreditati", "dare", "avere")
                for h in headers
            )
